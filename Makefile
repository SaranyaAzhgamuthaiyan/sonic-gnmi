...
$(BUILD_GNOI_YANG_PROTO_DIR)/.proto_api_done: $(API_YANGS)
	$(PYANG) -f proto --proto-outdir $(BUILD_GNOI_YANG_PROTO_DIR) --plugindir $(PYANG_PLUGIN_DIR) --server-rpc-outdir $(BUILD_GNOI_YANG_SERVER_DIR) --client-rpc-outdir $(BUILD_GNOI_YANG_CLIENT_DIR) -p $(MGMT_COMMON_DIR)/build/yang/common:$(MGMT_COMMON_DIR)/build/yang/extensions $(MGMT_COMMON_DIR)/build/yang/*.yang $(MGMT_COMMON_DIR)/build/yang/extensions/*.yang
	touch $@

$(BUILD_GNOI_YANG_PROTO_DIR)/.proto_sonic_done: $(SONIC_YANGS)
	$(PYANG) -f proto --proto-outdir $(BUILD_GNOI_YANG_PROTO_DIR) --plugindir $(PYANG_PLUGIN_DIR) --server-rpc-outdir $(BUILD_GNOI_YANG_SERVER_DIR) --client-rpc-outdir $(BUILD_GNOI_YANG_CLIENT_DIR) -p $(MGMT_COMMON_DIR)/build/yang/common:$(MGMT_COMMON_DIR)/build/yang/sonic/common $(MGMT_COMMON_DIR)/build/yang/sonic/*.yang
	touch $@
...
$(GOBIN)/protoc-gen-go:
	cd $$(mktemp -d) && $(GO) mod init protoc && $(GO) install github.com/golang/protobuf/protoc-gen-go
...
deinstall:
	rm -f $(DESTDIR)/usr/sbin/telemetry
ifneq ($(ENABLE_DIALOUT_VALUE),0)
	rm -f $(DESTDIR)/usr/sbin/dialout_client_cli
	rm -f $(DESTDIR)/usr/sbin/dialout_server_cli
endif
	rm -f $(DESTDIR)/usr/sbin/gnmi_cli
	rm -f $(DESTDIR)/usr/sbin/gnmi_get
	rm -f $(DESTDIR)/usr/sbin/gnmi_set
	rm -f $(DESTDIR)/usr/sbin/gnoi_client
	rm -f $(DESTDIR)/usr/sbin/gnmi_dump
